    ...
    $.ajax({
        ...
        success: function (products) {
            ...
            for (var i = 0; i < products.length; i++) {
                ...
                $('<td/>', {
                    html: '<button class="btn btn-success" style="padding-top: 5px; padding-bottom: 5px;" onclick="AddToOrder(this)"><b>+</b></button>'
                }).appendTo('#' + products[i].id);
            }
        }
    });
...
function SetOrderHeader(button) {
        ...
        $.ajax({
            ...
            success: function (orderedProducts) {
                for (let i = 0; i < orderedProducts.length; i++) {
                    ...
                    $('<td/>', {
                        html: '<button class="btn btn-success" style="padding-top: 5px; padding-bottom: 5px;" onclick="AddOneMore(this)"><b>+</b></button>'
                    }).appendTo('.' + "orderRowWithID" + orderedProducts[i].iD_Produkt);
                    $('<td/>', {
                        html: '<button class="btn btn-danger" style="padding-top: 5px; padding-bottom: 5px; padding-left: 14px; padding-right: 14px;" onclick="DeleteOne(this)"><b>-</b></button>'
                    }).appendTo('.' + "orderRowWithID" + orderedProducts[i].iD_Produkt);
                }
            }
        });
    ...
    $(".orderHeader").html("<div style='float: left;'><span><b>Tisch-Nr. " + tableOrderNumber + "</b></span></div><div style='float: right;'><button type='button' class='btn btn-warning' onclick='GoToStart()'><b>Zurück</b></button></div>");
    CalculateSum();
    SetSubTotalToZero();
    productsInSubTotal = [];
}
...
function CalculateSum() {
    ...
    $("#totalAmountOverview").html(totalAmount.toFixed(2) + " €");
}
...
function GoToTable() {
    document.getElementById("overviewOfTable").style.display = "inline-block";
    document.getElementById("overviewOfProducts").style.display = "none";
    document.getElementById("seeTableInfo").style.display = "none";
}

function GoToStart() {
    document.getElementById("overviewOfTable").style.display = "none";
    document.getElementById("overviewOfProducts").style.display = "inline-block";
    document.getElementById("seeTableInfo").style.display = "inline";
}